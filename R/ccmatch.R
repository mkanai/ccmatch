...
#' 
#' @param x.case a numeric \code{matrix}.
#' @param x.control a numeric \code{matrix}.
#' @param n a ratio of case:control.
#' @param display_progress a logical indicating whether to display a progress bar.
...
#' @export
ccmatch <- function(x.case, x.control, n = 1, display_progress = TRUE, method = "Euclidean", ...) {
    ...
    ret <- list()
    ret$ccmatch <- .ccmatch(x.dist, n, display_progress)
